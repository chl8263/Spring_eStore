...
public class CartRestController {
	...
	public ResponseEntity<Void> addItem(@PathVariable(value = "productId") int productId) {
		...
		for (int i = 0; i < cartItems.size(); i++) { // 기존의 카트에 있는지 확인함
			if (product.getId() == cartItems.get(i).getProduct().getId()) {
				CartItem cartItem = cartItems.get(i);
				cartItem.setQuantity(cartItem.getQuantity() + 1);
				cartItem.setTotalprice(product.getPrice() * cartItem.getQuantity());
				cartItemService.addCartItem(cartItem);
				System.out.println("!!!!!!!!!!!" + (product.getPrice() * cartItem.getQuantity()) + "");
	...
	public ResponseEntity<Void> minusItem(@PathVariable(value = "productId") int productId) {
		...
		for (int i = 0; i < cartItems.size(); i++) { // 기존의 카트에 있는지 확인함
			if (product.getId() == cartItems.get(i).getProduct().getId()) {
				...
				CartItem cartItem = cartItems.get(i);
				if (cartItem.getQuantity() >= 0 && cartItem.getProduct().getUnitInStock() >= cartItem.getQuantity()) {
					cartItem.setQuantity(cartItem.getQuantity() - 1);
					cartItem.setTotalprice(product.getPrice() * cartItem.getQuantity());
					cartItemService.addCartItem(cartItem);
					return new ResponseEntity<>(HttpStatus.OK);
